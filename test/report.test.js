...
test('the card shows a declared prerequisite and whether this machine has it', () => {
  ...
  const render = (setups) =>
    renderReport({
      ...
      claims: [claim],
      graph,
      doctorRes: { rows: [{ id: 'alpha', state: 'unknown', tier: null, issues: [], setups }], suspect: [], healthy: 1, errors: [] },
      inbox: [],
      generatedAt: '2026-01-01T00:00:00.000Z',
      version: '0.1.0',
    });
  const satisfied = render([{ name: 'node', run: 'npm ci', cached: true, missing: [] }]);
  assert.match(satisfied, /<dt>prerequisites<\/dt><dd><span class="muted">node → <\/span><code>npm ci<\/code>/);
  assert.match(satisfied, /satisfied here/);
  const pending = render([
    { name: 'node', run: 'npm ci', cached: true, missing: [] },
    { name: null, run: 'make deps', cached: false, missing: ['vendor'] },
  ]);
  ...
  assert.match(render([]), /<dt>prerequisites<\/dt><dd><span class="muted">—<\/span>/);
});
