...
- **The observe test fixture inherited this repo's real trust history.** `tmp()` copied `demo/.manual` wholesale, including the committed `ledger.jsonl` — written by whichever machine ran the tests. Locally the dedup hid it; on ubuntu the runner is a machine the ledger never met, so observe saw a phantom second machine and the single-machine test failed. Fixtures start with no ledger now; tests that study the ledger write their own. Found by the push CI the release triggered.
- **The canary matrix was replaced where upstream moved.** npm/cli's lockfile stayed out of sync at their HEAD (`npm ci` fails before any verifier runs — drift), react-router's HEAD migrated to pnpm (`packageManager: yarn@pnpm@11.7.0` — the yarn-1 tree no longer exists there), and httpx's Python lifecycle proposal was honestly `broken` (their suite passes only through their no-dependency test helper, so a plain-pytest claim cannot hold). The cells are now microsoft/TypeScript, react-router pinned at the commit the local drill validated, and flask for the Python lifecycle (preinstalled before the lifecycle runs, because its cold suite cannot fit the probe's calibration window).
