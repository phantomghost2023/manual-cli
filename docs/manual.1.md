...
`npm ls --depth=0` takes ~11s), `pnpm`, `yarn` (v1's `.yarn-integrity` map plus
the directories it linked; berry's `.yarn-state.yml` locations), `bun` (`bun.lock`
keys against the hoisted tree, or the `node_modules/.bun` store of the isolated
linker), `venv` (`requirements.txt`, `poetry.lock`, `Pipfile.lock`, `uv.lock`),
`gems` (`Gemfile.lock` against a vendored bundle), `gomod` (`go.mod` against
`$GOMODCACHE`, or `vendor/modules.txt`) and `crates` (`Cargo.lock` against
...
than assumed, which is why pnpm, crates and bun's isolated linker report a gap
instead of judging it: their package managers leave those states alone. It is
also why a classic yarn 1 repo's discovered install asks for `--check-files` —
plain `yarn install` answers "Already up-to-date" over a tree that lost a
package, and that flag re-links it in the same 0.2s. A cache entry whose declared
