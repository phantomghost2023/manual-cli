...
| `observe` | Reads verify history in `state.json`, writes inbox candidates when measurements diverge from claim bounds: *tighten* (the bound sits far above what runs cost), *raise* (the suite grew slower and the bound no longer fits the tail — proposed while the claim is still fresh, before a legitimate run fails), or *relax* (the bound itself broke the claim). Proposed bounds respect the tail — at least 3× the median, 1.5× p90, and 1.1× the worst run seen — and it refuses to propose a tightening the observed tail would violate. Every proposal also rewrites the claim's recorded `measured_ms` from the accumulated history, so the number next to the bound is not the single probe init took the day the claim was written. Each carries a *diagnosis* of why the series is spiky (cold start, trend, outlier, two clusters, correlated with free memory or machine load, cold-cache after an evidence change, or a single test dominating the suite), because naming the cause changes the right action. It also reports candidates that have gone *stale* (evidence moved since they were written). |
...
| `init [--dry-run]` | Inspects package.json, lockfiles, test scripts, migrations, CODEOWNERS — and beyond Node: go.mod + `_test.go` globs, vendored Go trees, pyproject/requirements/uv.lock and declared pytest — then scaffolds `.manual/`, a GitHub Actions workflow, and inbox candidates marked `origin: observed`, each probed once so its time bound is measured rather than invented. |
...
```bash
npm test        # 277 tests across 52 suites (seeded fuzzing, real-git integration, HTTP end-to-end)
node test/wild.mjs lifecycle --root <cloned-repo>   # canary: discovery + verify on a repo this tool didn't grow up with
